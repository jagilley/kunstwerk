...
`kunstwerk.py` is a thin orchestrator that shells out to each stage script (with the venv's bin dir prepended to `PATH`, so it works from cron/launchd). Stages are independently runnable and idempotent: `fetch_libretto.py`/`translate.py` refuse to overwrite without `--force`, `download_album.py` skips tracks already on disk, transcribers skip tracks whose JSON exists. Cheap, likely-to-fail stages (libretto lookup, translation, album resolution) run before the expensive ones (demucs, transcription, render).
