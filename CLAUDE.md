...
- API keys live in `.env` (gitignored), loaded via `python-dotenv`: `ELEVENLABS_API_KEY` (primary ASR, `scribe_v1` on the vocal stems), `OPENAI_API_KEY` (Whisper-1, *fallback only* for tracks ElevenLabs fails on — as of 2026-08 this key has no credits, so `transcribe.py` exits with an "add credits" message if it ever has work to do), plus unused legacy ones. **ASR is the weakest link**: Scribe sometimes returns hallucination loops (a Carmen track came back as 2,858× "la"; another as 1,009× "bon") or nothing at all (0 words on Carmen track 24, twice); re-transcribing fixes some tracks (19) and not others (24). `transcribe_elevenlabs.py` only retries on *empty* results, so loops slip through and the alignment stage has to strip them. A transcription quality gate (expected vocal seconds from `instrumental.json` vs. transcript coverage, loop detection) plus a credit-free Whisper fallback on Modal is the obvious next piece of work. **Claude is not called through an API key**: `translate.py` shells out to the Claude Code CLI (`claude -p`), which is billed to the subscription the CLI is logged into, and strips `ANTHROPIC_API_KEY` from that subprocess's environment. `ANTHROPIC_API_KEY` is deliberately commented out in `.env`; keep it that way. If `translate.py` says "Not logged in", run `claude` once and `/login`.
...
3. Writes `aligned_words_<prefix>.csv` and immediately reads it back — the hook for hand-editing timings — then `interpolate_word_timings` spreads times across untimed runs ≤ 20 s. `alignment_report` then writes `output/<prefix>-alignment-report.json` (coverage raw/confident, emulated black-screen fraction, gaps, per-track coverage/anchors/transcript holes, `review_reasons`, `notes`); **REVIEW NEEDED** when black > 20%, coverage < 40%, or any gap > 5 min. `--align-only` stops here; `kunstwerk.py` runs that as its `align` stage before the render and prints the banner (`--strict-alignment` makes it fatal). Carmen currently trips it (ASR holes); butterfly passes; giovanni trips only on one genuine 6-min hole.
